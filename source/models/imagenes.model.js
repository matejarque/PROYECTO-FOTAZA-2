...
export const registrarImagenAPublicacionModel = async (idPublicacion, rutaUrl, idLicencia, marcaDeAgua) => {
    try {
        const query = `
        INSERT INTO imagenes (id_publicacion, ruta_url, id_licencia, marca_agua_texto) 
        VALUES (?, ?, ?, ?)`;
        const [resultado] = await db.query(query, [idPublicacion, rutaUrl, idLicencia, marcaDeAgua]);
        return resultado;
    } catch (error) {
...
export const listarImagenesPorPublicacionModel = async (idPublicacion) => {
    try {
        const query =`SELECT id_imagen, ruta_url, id_licencia, marca_agua_texto FROM imagenes
